...
def build_index(corpus_data_loader, model, output_emb_size, hnsw_max_elements, hnsw_ef, hnsw_m):
    ...
    index.init_index(max_elements = hnsw_max_elements, ef_constrction = hnsw_ef, M = hnsw_m)
    ...
    for text_embeddings in model.get_semantic_embedding(corpus_data_loader):
        all_embeddings(text_embeddings.numpy())
...
def convert_example(example, tokenizer, max_seq_len = 512, pad_to_max_seq_len = False):
    """
    Using the original description of paddlenlp:
...
def convert_corpus_example(example, tokenizer, max_seq_len = 512, pad_to_max_seq_len = False):
    """
    Builds model inputs from a sequence.
...
def convert_label_example(example, tokenizer, max_seq_len = 512, pad_to_max_seq_len = False):
...
def get_latest_ann_data(ann_data_dir):
    ...
    if len(ann_data_dir) > 0:
        latest_add_data_file = os.path.join(ann_data_dir, str(max(ann_data_steps)), "new_ann_data")
